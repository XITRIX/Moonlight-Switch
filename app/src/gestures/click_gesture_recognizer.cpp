...
ClickGestureRecognizer::ClickGestureRecognizer(int fingersRequired, ClickGestureEvent::Callback respond)
    : fingers(fingersRequired)
{
    event.subscribe(respond);
}

GestureState ClickGestureRecognizer::recognitionLoop(TouchState touch, MouseState mouse, View* view, Sound* soundToPlay)
{
    ...
    else if (touch.phase == brls::TouchPhase::STAY)
    {
        ...
        if (position.x < startPosition.x - ALLOWED_RANGE || position.x > startPosition.x + ALLOWED_RANGE || position.y < startPosition.y - ALLOWED_RANGE || position.y > startPosition.y + ALLOWED_RANGE)
        {
