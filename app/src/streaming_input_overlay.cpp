...
void StreamingInputOverlay::draw(NVGcontext* vg, float x, float y, float width, float height, Style style, FrameContext* ctx)
{
    ...
    if (!isKeyboardOpen)
    {
        ...
        // Dead zone prevents from mouse drifting
        if (scroll_y < 0.2f && scroll_y > -0.2f) scroll_y = 0;
        ...
        if (l_pressed != old_l_pressed)
        {
            ...
            LiSendMouseButtonEvent(l_pressed ? BUTTON_ACTION_PRESS : BUTTON_ACTION_RELEASE, lb);
        }
        ...
        if (r_pressed != old_r_pressed)
        {
            ...
            LiSendMouseButtonEvent(r_pressed ? BUTTON_ACTION_PRESS : BUTTON_ACTION_RELEASE, rb);
        }
